function [output] = sbox_perm(xored)
% xored input - 1 x 48 
% output - 1 x 32
% This function gives corresponding sbox permutation of 32 bits for given
% 48 bits xored input(from previous DES step)
...
for i=1:8
    start = (i-1)*6 + 1;
    finish = i*6;
    row = bi2de([xored(start),xored(finish)],'left-msb')+ 1;
    column = bi2de(xored(start+1 : finish-1),'left-msb')+ 1;
    value = de2bi(sbox{i}(row,column),'left-msb',4); % getting 4 bits from each box 
    output((i-1)*4+1 : i*4) = value;
end
end
